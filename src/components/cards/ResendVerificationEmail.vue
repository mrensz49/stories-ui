<template>
  <v-col
    ...
    lg="12"
    md="12"
    v-if="authStore.is_logged_in && !authStore.user.email_verified_at"
  >
    <v-alert
      border="bottom"
      colored-border
    ...
    >
      ...
      <p class="text-justify">
        ...
        on the link we just emailed to you? <br />
      </p>
      If you didn't receive the email: <br />
      <span>&#8226;</span> Check your email
      <span class="font-italic font-weight-bold text-decoration-underline">{{
      ...
      <v-btn
        v-if="!authStore.verification_loading && !authStore.verified"
        color="warning"
        class="btn-sm ml-4 mb-3"
        :disabled="authStore.verification_loading"
        @click="authStore.handleResendVerification()"
      >
        Resend verification
      </v-btn>
      ...
      <p style="color: #b71c1c" v-if="authStore.errors_verification.length">
        {{ authStore.errors_verification }}
      </p>
    </v-alert>

    <p style="color: #b71c1c">
      Note: Dashboard is accessable only if email is verified.
    </p>
  </v-col>
</template>
